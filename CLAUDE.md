...
- Login is OAuth-only: `LoginView` renders a button per enabled provider (GitHub/Google/Microsoft, from `GET /api/auth-providers`) via Better Auth's `authClient.signIn.social`. `AuthContext` derives `currentUserId`/`currentUser` entirely from the real session (`authClient.useSession()`) — there's no local/demo login path. `roster` (used for board visibility and avatars) is `DEFAULT_ROSTER`'s decorative demo members plus every real registered user, merged in automatically the moment they show up in the `/api/members` query — real OAuth users default to the `member` role and get zero board/workspace access until an admin grants it in `AdminView`. `removeMember`/role overrides in `AuthContext` are session-local (not persisted).
